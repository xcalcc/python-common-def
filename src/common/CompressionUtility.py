...
class CompressionUtility:
    ...
    def get_archive(logger: XcalLogger, filename, file_path, input_file=None, destination_path=None):
        ...
        with XcalLogger("CompressionUtility", "get_archive", parent = logger) as log:
            ...
            log.set_tag("jaeger_tag", "archive_sourcecode")
            log.info("get_archive ", "begin to archive: %s" % file_path)

            if input_file is None or not os.path.exists(input_file):
                # if input_file(source_files.json) is None or not exists, collect all the source code files in project_path. For now scan java gradle projects need this.
                with zipfile.ZipFile(filename, 'w') as source_code_zip:
                    utility.goto_dir(file_path)
                    for root, dirs, filenames in os.walk(file_path, followlinks = True):
                        for filename in filenames:
                            if filename.endswith(".java")\
                                ...
                                source_code_zip.write(os.path.relpath(os.path.join(root, filename)))
                    utility.goback_dir()
            else:
