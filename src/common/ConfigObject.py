...
class ConfigObject(object):
    def __init__(self, *args):
        ...
        """
        if len(args) > 1:
            raise XcalException("ConfigObject", "__init__", "too many parameters, len of args = %d" % len(args),
    ...
    def prune_config(self):
        if isinstance(self.get("configContent"), str):
...
class UserConfigObject(ConfigObject):
    def __init__(self, user_config: str):
